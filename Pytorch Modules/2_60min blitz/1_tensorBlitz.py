...
x_rand = torch.rand_like(
    x_data, dtype=torch.float)  # overrides the fact we have int in data
...
# will print cpu, as i do not have a gpu on my laptop
...
# will concatenate the tensor 3 times, along the 1st axis
catTensor = torch.cat([tensor, tensor, tensor], dim=1)
print(catTensor)
